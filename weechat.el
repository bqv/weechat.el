            ...
            (let ((prefix-string (make-string (- (point-max) (point-min)) ?\s))
              ...
              (let* ((text (weechat-> text
                ...
                (insert (cond
                         ...
                         ((eq line-type :irc/privmsg)
                          (propertize text 'face 'weechat-privmsg-face))
                         ((eq line-type :irc/action)
                          (propertize text 'face 'weechat-action-face))
                         ((eq line-type :irc/notice)
                          (propertize text 'face 'weechat-notice-face))
                         ((eq line-type :irc/join)
                          (propertize text 'face 'weechat-join-face))
                         ((eq line-type :irc/part)
                          (propertize text 'face 'weechat-part-face))
                         ((eq line-type :irc/quit)
                          (propertize text 'face 'weechat-quit-face))
                         ((eq line-type :irc/mode)
                          (propertize text 'face 'weechat-mode-face))
                         ((eq line-type :irc/nick)
                          (propertize text 'face 'weechat-nick-face))
                         ((eq line-type :irc/topic)
                          (propertize text 'face 'weechat-topic-face))
                         ((eq line-type :irc/numeric)
                          (propertize text 'face 'weechat-numeric-face))
                         (t
                          (propertize text 'face 'weechat-default-face)))
                        "\n"))
